        ...
        public FeedStatus Status
        {
            ...
            set
            {
                ...
                if (feedStatus == FeedStatus.Available)
                {
                    ...
                    if (attempt==0)
                    {
                        Logger.InfoFormat("Subscribed for feed on channel '{0}', Context: {1} ({2}) ", _channelName, _context, typeof(TContext));
                    }else
                    {
                        Logger.InfoFormat("Subscription for feed on channel '{0}', Context: {1} ({2}) is restored after {3} attempts", _channelName, _context, typeof(TContext), attempt);
                    }
                }

                if (feedStatus == FeedStatus.NotSubscribed )
                 {
                     Logger.InfoFormat("Unsubscribed from feed on channel '{0}', Context: {1} ({2}) ", _channelName, _context, typeof(TContext));
                 }
            }
        }
